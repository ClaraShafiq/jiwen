...
> **这不是一个聊天机器人。** 这是一个 ~500 行的纯函数。它管理五个连续心理轴的数学漂移，到阈值了告诉你「角色现在想开口」「角色在找借口逃避」「角色心情差到需要自我调节」——但不管开口说什么、找什么借口、怎么调节。
>
> 你带自己的 LLM、自己的数据库、自己的角色人设。积温只做一件事：在后台持续算着那几个数字。**它是引擎，不是车。**

## 这是什么

把「角色什么时候会主动说话」从概率骰子换成连续数学。

- **`createJiwen(opts)`** — 工厂函数，返回一个引擎实例。零依赖，纯 JS
- **`tick(minutes)`** — 推进状态漂移，返回触发动作数组。不调 LLM，纯数学
- **`getPromptContext()` / `getStyleGuidance()`** — 把五个数字翻译成自然语言，注入你的 LLM prompt
- **`simulate.js`** — 给定事件线和多组参数，输出状态轨迹 CSV + 诊断列，校准用
- **29 项测试** — `node jiwen.test.js`，覆盖单调性、边界、阈值转移、回归

积温不管记忆、不管对话生成、不管网络请求。它是一颗跳动的数字心脏，你把它缝进角色的胸腔里。
...
三层叠加让等待的感受不是均匀的：对方说了晚安 → 涨得慢（不急），突然中断 → 涨得快（挂念更重）。等了半小时还没动静 → 开始加速。心情差想求安慰 → 涨更快；心情烂透了自我封闭 → 涨放慢。

### 心理动力学参数（默认关闭，向后兼容）

| 参数 | 作用 | 心理依据 |
|------|------|---------|
| `connectionAccel` | 非线性加速指数 | 越等越焦虑——不是线性累积，是加速攀升 |
| `accelDelay` | 加速前的线性缓冲（分钟） | 刚聊完还不急，过一阵才开始加速 |
| `valenceSetpoint` | Valence 回归目标（默认 0） | 角色自然情绪基调——有些人偏冷，有些人偏暖 |
| `valenceConnectBoost/Threshold` | 轻度不开心时 connection 加速 | 想要安慰——越被冷落越想找她 |
| `valenceConnectDampen/Threshold` | 严重低落时 connection 减速 | 自我封闭——太难过了反而缩回去 |
